...
class Camera:
    ...
    def update(self):
        ...
        cameraRotation = pyrr.matrix44.create_from_eulers(
            eulers = np.radians(self.eulers), 
            dtype=np.float32
        )
        self.up = pyrr.matrix44.multiply(
            m1 = self.localUp,
            m2 = cameraRotation
        )
        self.right = pyrr.matrix44.multiply(
            m1 = self.localRight,
            m2 = cameraRotation
        )
        self.forwards = pyrr.matrix44.multiply(
...
class Scene:
    ...
    def spin_camera(self, dEulers):
        ...
        self.camera.eulers[2] = min(179, max(1, self.camera.eulers[2]))
