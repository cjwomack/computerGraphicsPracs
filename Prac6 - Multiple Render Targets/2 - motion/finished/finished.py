...
GRAVITY = np.array([0,0,-0.01], dtype=np.float32)
...
class Billboard:
    """
        A billboard object, represents a region of heat haze.
    """
    ...
    def __init__(self, position: list[float], size: list[float]):
        ...
        self.type = OBJECT_HAZE
    
    def get_model_transform(self, viewerPos: np.ndarray) -> np.ndarray:

        directionFromPlayer = self.position - viewerPos
        angle1 = np.arctan2(-directionFromPlayer[1],directionFromPlayer[0])
        dist2d = np.sqrt(directionFromPlayer[0] ** 2 + directionFromPlayer[1] ** 2)
        angle2 = np.arctan2(directionFromPlayer[2],dist2d)

        model_transform = pyrr.matrix44.create_identity(dtype=np.float32)
        model_transform = pyrr.matrix44.multiply(
            model_transform,
            pyrr.matrix44.create_from_scale(scale=self.size, dtype=np.float32)
        )
        model_transform = pyrr.matrix44.multiply(
            model_transform,
...
class Ball(Entity):
    ...
    def __init__(self, position: list[float]):
        ...
        super().__init__(position, [0,0,0], OBJECT_BALL)
...
class Scene:
    ...
    def __init__(self):
        ...
        self.renderables[OBJECT_GROUND] = [
            Entity(
                position=[10,0,-1],
                eulers=[0,0,0],
                objectType=OBJECT_GROUND
            )
        ]

        self.renderables[OBJECT_GIANT] = [
            Entity(
                position=[40,0,-20],
                eulers=[0,0,0],
                objectType=OBJECT_GIANT
            )
        ]
        self.renderables[OBJECT_BALL] = [
            Ball(
                position = [np.random.uniform(0,20), np.random.uniform(-10,10), np.random.uniform(1,10)]
            )
            for _ in range(8)
        ]
        self.hazeRegions: list[Billboard] = [Billboard(position = [6,0,2], size = [2,2,2])]
    ...
    def mainLoop(self) -> None:
        ...
        while (running):
            ...
            self.renderer.render(
                ...
                hazeRegions = self.scene.hazeRegions
            )
...
class Renderer:
    ...
    def make_assets(self) -> None:
        ...
        self.meshes: dict[int, Mesh] = {
            ...
            OBJECT_HAZE: ObjMesh("models/quad.obj"),
            OBJECT_GROUND: ObjMesh("models/ground.obj"),
            OBJECT_GIANT: ObjMesh("models/giant.obj"),
            OBJECT_BALL: ObjMesh("models/ball.obj")
        }

        self.materials: dict[int, Material] = {
            OBJECT_CUBE: Material2D("gfx/wood.jpeg"),
            OBJECT_HAZE: Material2D("gfx/explosion.jpg"),
            OBJECT_GROUND: Material2D("gfx/earth.jpg"),
            OBJECT_SKY: MaterialCubemap("gfx/sky"),
            OBJECT_GIANT: Material2D("gfx/steel.jpg"),
            OBJECT_BALL: Material2D("gfx/red.jpg")
        }
        ...
        self.framebuffers[LAYER_STANDARD] = Framebuffer(
            colorAttachments = [self.colorBuffers[0], self.colorBuffers[1]], 
            depthBuffer = self.depthBuffer
        )
        self.framebuffers[LAYER_EFFECTS] = Framebuffer(
            colorAttachments = [self.colorBuffers[1]], 
            depthBuffer = self.depthBuffer
        )

        self.screenQuad = Quad2D(center=(0,0), size=(1,1))
        ...
        self.shaders: dict[int, int] = {
            ...
            PIPELINE_POST: createShader(
                "shaders/vertex_post.txt", 
                "shaders/fragment_post.txt"
            )
        }
    ...
    def set_onetime_uniforms(self) -> None:
        ...
        glUniform1i(
            glGetUniformLocation(self.shaders[PIPELINE_POST], "colorbuffer"), 0)
        glUniform1i(
            glGetUniformLocation(self.shaders[PIPELINE_POST], "fxbuffer"), 1)
    ...
    def get_uniform_locations(self) -> None:
        ...
        self.velocityLocation = glGetUniformLocation(
            self.shaders[PIPELINE_3D], "velocity"
        )
    ...
    def render(
        ...
        hazeRegions: list[Billboard]) -> None:
        """
            Render a frame.
        ...
        #regular 3D rendering to our standard framebuffer
        glBindFramebuffer(GL_FRAMEBUFFER, self.framebuffers[LAYER_STANDARD].fbo)
        ...
        for objectType,objectList in renderables.items():
            ...
            for object in objectList:
                ...
                glUniform3fv(self.velocityLocation, 1, object.velocity)
                glDrawArrays(GL_TRIANGLES, 0, mesh.vertex_count)
        
        #regular 3D rendering to our specialfx framebuffer
        #glBindFramebuffer(GL_FRAMEBUFFER, self.framebuffers[LAYER_EFFECTS].fbo)
        #glEnable(GL_DEPTH_TEST)
        ...
        self.materials[OBJECT_HAZE].use()
        glBindVertexArray(self.meshes[OBJECT_HAZE].vao)
        for object in hazeRegions:
            glUniformMatrix4fv(
                self.modelMatrixLocation,
                1,GL_FALSE,
                object.get_model_transform(camera.position)
            )
            glDrawArrays(GL_TRIANGLES, 0, mesh.vertex_count)
        """
...
class DepthStencilbuffer:
    ...
    def destroy(self) -> None:
        """ Free allocated space """
        glDeleteRenderbuffers(1, (self.texture,))
