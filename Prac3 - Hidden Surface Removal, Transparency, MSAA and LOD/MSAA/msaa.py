...
class Renderer:
    ...
    def __init__(self, screenWidth, screenHeight):
        ...
        self.post_processing_shader = createShader("shaders/vertexPost.txt", "shaders/fragmentPost.txt")
    ...
    def create_framebuffer(self):
        """
            Create a custom framebuffer so we can draw anti-aliased stuff
        """

        samples = 1
        #create a framebuffer object
        self.multisampleFBO = glGenFramebuffers(1)
        glBindFramebuffer(GL_FRAMEBUFFER, self.multisampleFBO)
        
        #and a color buffer for it
        self.multisampleColorBuffer = glGenTextures(1)
        glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, self.multisampleColorBuffer)
        #reserve space to hold the colorbuffer
        glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, samples, GL_RGB, self.screenWidth, self.screenHeight, GL_TRUE)
        #we're done with specifying our colorbuffer, unbind it.
        glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, 0)
        #attach the colorbuffer to the framebuffer object
        glFramebufferTexture2D(
            GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D_MULTISAMPLE, self.multisampleColorBuffer, 0
        )
        
        #now create a depth/stencil buffer for the framebuffer object
        self.multisampleDepthStencilBuffer = glGenRenderbuffers(1)
        glBindRenderbuffer(GL_RENDERBUFFER, self.multisampleDepthStencilBuffer)
        glRenderbufferStorageMultisample(GL_RENDERBUFFER, samples, GL_DEPTH24_STENCIL8, self.screenWidth, self.screenHeight)
        glBindRenderbuffer(GL_RENDERBUFFER,0)
        #similarly, attach it to the framebuffer
        glFramebufferRenderbuffer(
            GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, self.multisampleDepthStencilBuffer
        )

        #we can't draw a multisample colorbuffer directly, it must be
        # "resolved" to a single sample colorbuffer.
        self.singlesampleFBO = glGenFramebuffers(1)
        glBindFramebuffer(GL_FRAMEBUFFER, self.singlesampleFBO)
        
        self.singlesampleColorBuffer = glGenTextures(1)
        glBindTexture(GL_TEXTURE_2D, self.singlesampleColorBuffer)
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, self.screenWidth, self.screenHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, None)
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR)
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR)
        glBindTexture(GL_TEXTURE_2D, 0)
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, 
                                GL_TEXTURE_2D, self.singlesampleColorBuffer, 0)
    ...
    def load_assets(self):
        ...
        glUseProgram(self.post_processing_shader)
        self.meshes["screen"] = QuadMesh(L=1,W=1)
    ...
    def render(self, scene):
        ...
        for triangle in scene.triangles:
            ...
            glUniform4fv(self.objectColorLocation, 1, triangle.color)
        ...
        """
            glBlitFramebuffer(
                source,
                destination,
                mask (what data to copy?), filter (if squash/stretch needs to be applied)
            )
        """
        glBlitFramebuffer(
    ...
    def destroy(self):
        ...
        glDeleteProgram(self.post_processing_shader)
        glDeleteTextures(2, (self.multisampleColorBuffer, self.singlesampleColorBuffer))
        glDeleteRenderbuffers(1, (self.multisampleDepthStencilBuffer,))
        glDeleteFramebuffers(2, (self.multisampleFBO, self.singlesampleFBO))
        pg.quit()
